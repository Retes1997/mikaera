...
* *Delegación y Exclusiones de Clics en Grillas:* Al programar interacciones sobre tarjetas que contienen enlaces alternativos (como `proyecto-detalle.html` para SEO y navegación sin JS) y sub-acciones internas (como botones `+` para zoom en Lightbox), el escuchador de eventos debe capturar el clic sobre el elemento contenedor principal (`.portfolio-item`) y detener su comportamiento nativo (`e.preventDefault()`). Las acciones secundarias anidadas deben utilizar `e.stopPropagation()` o filtrarse explícitamente mediante `.closest()` para prevenir que se dispare la redirección o el modal general de forma cruzada.

### 3. Rendimiento Web y Optimización de Renderizado (CLS, Scrollbars & Performance)
...
* **Control de Saltos por Scrollbar (Stable Scrollbar Gutter):** Para evitar que el diseño completo se desplace horizontalmente unos píxeles al cambiar entre una página con mucho scroll y otra corta, la etiqueta `html` tiene declarada la propiedad `scrollbar-gutter: stable;` en [reset.css](file:///c:/Users/Retes/Documents/webs/primo-andrez/proyecto-web/css/base/reset.css). Esto reserva un canal constante en el lateral derecho para la barra de desplazamiento, aislando el área de contenido y manteniendo la simetría visual en todos los navegadores.
* **Evitar Ancho Viewport en Flujo (No 100vw):** Queda prohibido usar `width: 100vw` en elementos dentro del flujo regular de la página (como la sección Hero o bloques promocionales). En navegadores con barras de desplazamiento físicas, `100vw` abarca la barra entera, lo que provoca desbordamiento horizontal y scrollbars secundarios no deseados. Se debe utilizar siempre `width: 100%` para acomodar el elemento de forma segura en el espacio disponible.
